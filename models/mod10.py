...
class KPFCNN(nn.Module):
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            if layer_count>0 and 'strided' in block:
                ...
                for num_block in range(layer_count):
                    self.layer_path.append(block_decider('simple',
                                                        r,
                                                        (in_dim*2)+(out_dim*mul_var),#change happened here 
                                                        out_dim*2,
                                                        layer,
                                                        config))
                    #mul_var+=1 #for the first layer it should create 3 blocks
                    #don't wanna change the in_dim when not using dense,
                layer_count-=1
    ...
    def forward(self, batch, config):
        ...
        x = self.encoder_blocks[2](x0_0,batch)#strided
        #x = self.encoder_blocks[3](x,batch)
        x1_0 = self.encoder_blocks[3](x,batch)
        x0_1 = self.path_blocks[0][0](torch.cat([x0_0, self.upsample_path_blocks[0][0](x1_0,batch)],dim=1),batch)
        #segout append here

        x = self.encoder_blocks[4](x1_0,batch)#check right after encoder
        #x = self.encoder_blocks[6](x,batch)
        x2_0 = self.encoder_blocks[5](x,batch)
        x1_1 = self.path_blocks[1][0](torch.cat([x1_0,self.upsample_path_blocks[1][0](x2_0,batch)],dim=1),batch)
        x0_2 = self.path_blocks[0][1](torch.cat([x0_1,self.upsample_path_blocks[0][1](x1_1,batch)],dim=1),batch)
        #segout here

        x = self.encoder_blocks[6](x2_0,batch)#right after
        #x = self.encoder_blocks[9](x,batch)
        x3_0 = self.encoder_blocks[7](x,batch)
        x2_1 = self.path_blocks[2][0](torch.cat([x2_0, self.upsample_path_blocks[2][0](x3_0,batch)],dim=1),batch)
        x1_2 = self.path_blocks[1][1](torch.cat([x1_1, self.upsample_path_blocks[1][1](x2_1,batch)],dim=1),batch)
        x0_3 = self.path_blocks[0][2](torch.cat([x0_2, self.upsample_path_blocks[0][2](x1_2,batch)],dim=1),batch)
        #segout here

        x = self.encoder_blocks[8](x3_0,batch)
        #x = self.encoder_blocks[12](x,batch)
        x4_0 = self.encoder_blocks[9](x,batch)#last block of encoder
        ...
        x2_2 = self.decoder_blocks[3](torch.cat([x2_1,x],dim=1),batch)
        x = self.decoder_blocks[4](x2_2,batch)#upsample
        x1_3 = self.decoder_blocks[5](torch.cat([x1_2,x],dim=1),batch)
        x = self.decoder_blocks[6](x1_3,batch)
        x0_4 = self.decoder_blocks[7](torch.cat([x0_3,x],dim=1),batch)
        #segout here
